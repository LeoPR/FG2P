...
def load_metadata(model_name: str) -> Optional[dict]:
    """Carrega _metadata.json de um modelo (busca em subpasta por experimento)."""
    exp_name      = model_name.split("__")[0]
    metadata_path = MODELS_DIR / exp_name / f"{model_name}_metadata.json"
    if not metadata_path.exists():
        # Fallback path plano (legado)
