...
def _build_model_list() -> list:
    """Constrói lista de modelos usando file_registry — fonte única de verdade."""
    ...
    for rec in list_experiments(complete_only=True):
        models.append({
            ...
            "predictions_file": rec.predictions_path if rec.has_predictions else None,
            "has_predictions":  rec.has_predictions,
        })
    return models


def list_available_models():
    """Lista todos os modelos com índices consistentes com inference.py --list."""
    ...
    for model in models:
        ...
        pred_status = "[OK] Predicoes disponíveis" if model["has_predictions"] else "[X] Sem predicoes"
...
def select_model(args):
    """Seleciona modelo baseado nos argumentos — índices idênticos a inference.py."""
    all_models   = _build_model_list()
    models_preds = [m for m in all_models if m["has_predictions"]]
    
    if not all_models:
        logger.error("Nenhum modelo encontrado.")
        logger.info("Execute train.py primeiro.")
        return None

    # Seleção por --index (usa índice global, consistente com inference.py --list)
    if args.index is not None:
        if 0 <= args.index < len(all_models):
            m = all_models[args.index]
            if not m["has_predictions"]:
                logger.error(f"Modelo [{args.index}] não tem predições. Execute inference.py --index {args.index} primeiro.")
                return None
            return m["predictions_file"]
        else:
            logger.error(f"Índice {args.index} inválido. Use 0-{len(all_models) - 1} (mesmo que inference.py --list).")
            return None
    ...
    if args.model:
        for m in all_models:
            if args.model in m["name"]:
                if not m["has_predictions"]:
                    logger.error(f"Modelo '{m['name']}' sem predições. Execute inference.py --index {m['index']} primeiro.")
                    return None
                return m["predictions_file"]
        logger.error(f"Modelo '{args.model}' não encontrado")
        return None

    # Padrão: mais recente com predições
    if not models_preds:
        logger.error("Nenhum modelo com predições. Execute inference.py primeiro.")
        return None
    latest = models_preds[-1]
    logger.info(f"Usando modelo mais recente com predições: [{latest['index']}] {latest['name']}")
...
def calculate_per(predictions, references):
    ...
    Fórmula (Morris et al., 2004; Bisani & Ney, 2008):
        PER = sum_i( edit_dist(pred_i, ref_i) ) / sum_i( len(ref_i) ) * 100

    Denominador = comprimento total de REFERÊNCIA (não predição nem máximo).
    Usa distância de Levenshtein (inserção, deleção, substituição = custo 1).

    Args:
        predictions: Lista de sequências preditas (cada item é list[str] de fonemas)
        references:  Lista de sequências de referência (mesmo formato)
...
def calculate_accuracy(predictions, references):
    """Calcula Word-level Accuracy (acurácia exata por palavra).

    Fórmula:
        Acc = |{i : pred_i == ref_i}| / N * 100

    Contexto G2P: duas transcrições são consideradas idênticas somente se a
    sequência completa de fonemas coincide token a token (exact-match).
    Não é a acurácia de ASR por palavras — aqui a unidade é a palavra inteira
    e seu alinhamento de fonemas, não sequências de palavras em uma frase.

    Returns:
        Accuracy em % (float). 100.0 = todas as palavras perfeitas.
    """
    if not predictions:
        return 0.0
...
def calculate_wer(predictions, references):
    """Calcula Word Error Rate (WER = 100 - Accuracy).

    Em G2P, WER é a taxa de palavras com QUALQUER erro de fonema.
    Equivalente ao que a literatura chama de String Error Rate (SER) ou
    Sentence Error Rate — a palavra é a unidade de avaliação, e um único
    fonema errado conta como palavra errada inteira.

    Distinção importante: este WER difere do WER padrão de ASR (que usa
    distância de edição no nível de palavras numa frase). Em G2P, a unidade
    de entrada/saída já é a palavra, então WER_G2P = 1 - word_accuracy.

    Referências: Bisani & Ney (2008), Yao & Zweig (2015).

    Returns:
        WER em % (float). 0.0 = todas as palavras perfeitas.
    """
...
def wilson_ci(count: int, n: int, confidence: float = 0.95) -> tuple[float, float]:
    ...
    Preferível ao intervalo de Wald (p ± z*sqrt(p(1-p)/n)) porque é exato
    para proporções extremas (p próximo de 0 ou 1) e válido para n pequeno.

    Fórmula (Wilson, 1927; Brown, Cai & DasGupta, 2001):
        centro = (p̂ + z²/2n) / (1 + z²/n)
        margem = z/(1+z²/n) * sqrt(p̂(1-p̂)/n + z²/4n²)
        CI = [centro - margem, centro + margem]

    Args:
        count: Número de "erros" (ex: fonemas errados para PER)
        n:     Total de observações (ex: total de fonemas para PER)
        confidence: Nível de confiança (padrão 0.95 = 95%)

    Returns:
        (ci_lower, ci_upper) em % (não em proporção — multiplicado por 100)

    Exemplos típicos para FG2P:
        PER Exp9:    wilson_ci(1050, 181000)  → (~0.55%, 0.61%) [N fonemas]
        WER Exp104b: wilson_ci(1567, 28782)   → (~5.17%, 5.70%) [N palavras]
    """
    import math
    if n == 0:
        return 0.0, 100.0
    ...
    if confidence == 0.95:
        z = 1.95996  # scipy.stats.norm.ppf(0.975)
    elif confidence == 0.99:
        z = 2.57583
    else:
        # Aproximação suficiente para outros níveis
        import math
        # Usar valor tabelado padrão
        z = 1.95996
    p_hat = count / n
    z2 = z * z
    denom = 1 + z2 / n
...
def load_performance_data():
    ...
    try:
        ...
        logger.warning("Falha ao ler %s: %s", PERFORMANCE_PATH.name, exc)
        return None


def load_inference_stats(results_exp_dir: Path, base_name: str) -> dict:
    """Lê inference_stats.json gerado por inference.py."""
...
def format_benchmark_comparison(per, wer, accuracy, dataset_size, perf_data=None):
    """Formata lista de benchmarks para comparação com a literatura."""
    benchmarks = [{
        'name': 'FG2P (este run)',
        'lang': 'PT-BR',
    ...
    if perf_data:
        for section in ('fg2p_models', 'literature_ptbr', 'literature_general'):
            for entry in perf_data.get(section, []):
                benchmarks.append({
...
Exemplos:
  ...
  python src/analyze_errors.py --model exp2 # Analisa modelo específico
    ...
    metadata = load_model_metadata(MODELS_DIR / exp_name / f"{base_name}.pt")
    ...
    exp_name_report = base_name.split("__")[0]
    report_dir = RESULTS_DIR / exp_name_report
    report_dir.mkdir(exist_ok=True)
    report_file = report_dir / f"error_analysis_{base_name}.txt"
    ...
    # ── Salvar evaluation.txt ─────────────────────────────────────────────────
    infer_stats = load_inference_stats(report_dir, base_name)
    perf_data = load_performance_data()
    benchmarks = format_benchmark_comparison(per, wer, accuracy, total_words, perf_data)

    eval_file = report_dir / f"evaluation_{base_name}.txt"
    timestamp_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    exp_display = metadata.get('experiment_name', exp_name_report) if metadata else exp_name_report
    ...
    with open(eval_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Model: {base_name}.pt\n")
        f.write(f"Experiment: {exp_display}\n")
        if metadata:
            current_epoch = metadata.get('current_epoch', '?')
            total_epochs_meta = metadata.get('total_epochs', '?')
            is_complete = metadata.get('training_completed', False)
            f.write(f"Training Status: Epoch {current_epoch}/{total_epochs_meta} "
                    f"({'Complete' if is_complete else 'Incomplete'})\n")
        f.write(f"Test set: {total_words} words\n")
        if infer_stats:
            f.write(f"Inference time: {infer_stats['inference_time_s']:.2f}s "
                    f"({infer_stats['samples_per_sec']:.1f} samples/s)\n")
        f.write("\n")
        f.write("METRICS\n")
        f.write("-" * 70 + "\n")
        ...
        f.write(f"Correct words: {correct_count}/{total_words}\n")
        f.write("\n")
        ...
        for bm in benchmarks:
            ...
            f.write(f"{bm['name']:<28} {bm['lang']:<7} {bm['dataset']:<16} "
                    f"{per_str:<7} {wer_str:<7} {acc_str:<7}\n")
        ...
        if notes:
            ...
            f.write("\nNote: EN (irregular) vs PT-BR (regular) are not directly comparable.\n")

    logger.info("=" * 70)
    logger.info(f"Relatorio de erros: {report_file.name}")
    logger.info(f"Avaliacao: {eval_file.name}")
