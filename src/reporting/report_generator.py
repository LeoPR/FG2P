...
#HTML Generation
    ...
    html += f"""
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Total de palavras
                                <span class="tooltip-text">Número total de palavras únicas no dataset (train + val + test)</span>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Razão de split (Train/Val/Test)
                                <span class="tooltip-text">Proporção de palavras em cada partição.<br><br>
                                <strong>Train:</strong> Dados usados para treinar o modelo (~60-70%)<br>
                                <strong>Val:</strong> Dados usados para early stopping (~10%)<br>
                                <strong>Test:</strong> Dados nunca vistos, usados para avaliação final (~20-30%)<br><br>
                                <strong>⚠️ Importante:</strong> Os conjuntos são <strong>completamente disjuntos</strong> (sem sobreposição).<br>
                                Exp0 usa 70/10/20, Exp1-6 usam 60/10/30 (test maior para avaliação mais robusta).
                                </span>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Grafemas únicos
                                <span class="tooltip-text">Número de caracteres alfabéticos distintos no dataset (a-z, incluindo acentuadas)</span>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Fonemas únicos
                                <span class="tooltip-text">Número de símbolos fonéticos distintos no IPA (International Phonetic Alphabet)</span>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Comprimento médio (grafemas)
                                <span class="tooltip-text">Número médio de caracteres por palavra na ortografia original</span>
                            </span>
                        </td>
                    ...
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Comprimento médio (fonemas)
                                <span class="tooltip-text">Número médio de fonemas (sons) por palavra na transcrição IPA</span>
                            </span>
                        </td>
...
"""
    ...
    if metadata.get("computed_at"):
        ...
        html += f"""
                    <tr>
                        <td>
                            <span class="tooltip-trigger">Última atualização
                                <span class="tooltip-text">Timestamp da última computação das estatísticas (cache)</span>
                            </span>
                        </td>
...
"""
    ...
    # Comparação de Splits Usados nos Experimentos
    if unique_splits and len(unique_splits) > 0:
        html += """
                <!-- Comparação de Splits -->
                <h3 style="margin-top: 30px;">Configurações de Split Utilizadas</h3>
                <p style="color: #666; margin-bottom: 15px;">
                    Diferentes experimentos usam diferentes proporções de dados para treino/validação/teste.
...
"""
        ...
        html += """
                ...
                <div style="margin-top: 10px; padding: 12px; background: #e3f2fd; border-left: 4px solid #2196F3; border-radius: 4px; font-size: 0.9em;">
                    <strong>💡 Sobre os splits:</strong>
                    <ul style="margin: 8px 0 0 20px; padding: 0;">
                        <li><strong>70/10/20:</strong> Split original - mais dados para treinamento, teste moderado</li>
                        <li><strong>60/10/30:</strong> Split com teste ampliado - 50% mais palavras de teste para avaliação mais robusta</li>
                        <li>Todos os splits mantêm as mesmas 9.6k palavras de validação (10%)</li>
                        <li>Os conjuntos são <strong>completamente disjuntos</strong> (sem sobreposição)</li>
                    </ul>
                </div>
"""
    
    # Métricas de representatividade (se disponíveis)
    if repr_metrics:
...
"""
    ...
    for split_name in ["train", "val", "test"]:
        ...
        word_len_str = f"{word_len.get('min', 0)}-{word_len.get('max', 0)} (μ={word_len.get('mean', 0):.1f})"
        phoneme_len_str = f"{phoneme_len.get('min', 0)}-{phoneme_len.get('max', 0)} (μ={phoneme_len.get('mean', 0):.1f})"
...
"""
    ...
    html += """
                ...
                <h3>Distribuição de Fonemas (Top 15)</h3>
...
"""
    ...
    html += """
                ...
                <!-- Percentiles -->
                <h3>Distribuição de Comprimentos (Percentis)</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Split</th>
