...
Conteúdo baseado em docs/presentation/PRESENTATION.md com tags [modes: full|compact]
...
def slide_attention(prs: Presentation, num: int):
    ...
    add_code_box(s, Inches(0.6), Inches(2.0), Inches(5.5), Inches(1.4),
                 'Encoder: b-i-s-c-o-i-t-o → 1 vetor (384D)\n384 números resumem TODA a palavra\nDecoder: não consegue voltar\npara verificar letra 3 e 4 (sc)',
                 font_size=12)
    ...
    add_code_box(s, Inches(0.6), Inches(4.4), Inches(5.5), Inches(1.7),
                 'Encoder: 8 vetores (384D cada)\n  s(384D)  c(384D)  ...\nAtenção acessa todos:\n  s=0.70 + c=0.20\n  → reconhece padrão "sc"',
                 font_size=12)
...
def slide_da_example(prs: Presentation, num: int):
    ...
    code = (
        'PASSO 1 — Distribuição do modelo (softmax):\n'
        '  s=42%  ʃ=35%  z=12%  ...\n'
        '  Escolhe: ʃ (argmax) ← errado! esperado: s\n'
        '\n'
        'PASSO 2 — CrossEntropy:\n'
        '  p(s) = 0.42  ← probabilidade do correto\n'
        '  L_CE = −log(0.42) = 0.87\n'
        '\n'
        'PASSO 3 — Distância (ʃ ≠ s, mas próximos):\n'
        '  d(ʃ, s) = 0.15\n'
        '\n'
        'PASSO 4 — DA Loss extra:\n'
...
def build_presentation_from_markdown(md_path: Path, output_path: Path, filtered_markdown: str = None) -> Path:
    ...
    if is_compact:
        logger.info("  [MODO COMPACTO] Versão simplificada (10 minutos)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="MD_PATH",
        help="Arquivo Markdown customizado (padrão: docs/presentation/PRESENTATION.md)",
    )
    ...
    if args.markdown:
        ...
        markdown_file = Path(__file__).parent.parent.parent / "docs" / "presentation" / "PRESENTATION.md"
