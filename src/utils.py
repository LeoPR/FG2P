...
def list_model_artifacts(model_name: str) -> dict[str, Path | list[Path] | None]:
    ...
    """
    exp_name = model_name.split("__")[0]
    m_dir = MODELS_DIR / exp_name
    r_dir = RESULTS_DIR / exp_name

    artifacts = {
        "model_file": m_dir / f"{model_name}.pt",
        "metadata_file": m_dir / f"{model_name}_metadata.json",
        "history_csv": r_dir / f"{model_name}_history.csv",
        "summary_file": r_dir / f"{model_name}_summary.txt",
        "evaluation_files": [],
        "prediction_files": [],
        "analysis_plots": [],
    ...
    # Buscar em subpastas (padrão: results/{exp_name}/arquivo)
    artifacts["evaluation_files"] = list(RESULTS_DIR.glob(f"*/evaluation_{model_name}.txt"))
    artifacts["prediction_files"] = list(RESULTS_DIR.glob(f"*/predictions_{model_name}.tsv"))
    artifacts["error_analysis_files"] = list(RESULTS_DIR.glob(f"*/error_analysis_{model_name}.txt"))
...
def get_orphan_files() -> dict[str, list[Path]]:
    ...
    """
    # Listar todos os modelos existentes (busca recursiva em subpastas)
    ...
    # Evaluation e predictions órfãos
    for ef in RESULTS_DIR.glob("*/evaluation_*.txt"):
        model_name = ef.stem.replace("evaluation_", "")
        if model_name not in model_names:
            orphans["evaluation"].append(ef)

    for pf in RESULTS_DIR.glob("*/predictions_*.tsv"):
        model_name = pf.stem.replace("predictions_", "")
        if model_name not in model_names:
            orphans["predictions"].append(pf)

    # Plots órfãos
    for plot in RESULTS_DIR.glob("*/*.png"):
        found = any(plot.stem.startswith(mn) for mn in model_names)
        if not found:
