...
class G2PCorpus:
    ...
    def split(self, test_ratio: float = 0.2, val_ratio: float = 0.1,
        ...
        """
        Cria split 3-way: train / val / test.

        Se stratify=True (padrão): Estratifica por (tonicidade × sílabas × comprimento), garantindo
        que cada família linguística esteja representada proporcionalmente
        nos três subsets.

        Se stratify=False: Split aleatório simples (sem estratificação) — útil para ablation studies.
        ...
        # Estratificação: colapsar estratos com <2 exemplos (apenas se stratify=True)
        if stratify:
        ...
        trainval_idx, test_idx = train_test_split(
            ...
            stratify=strata if stratify else None,
        )
        ...
        train_idx, val_idx = train_test_split(
            ...
            stratify=trainval_strata if stratify else None,
        )
...
class G2PLSTMModel(nn.Module):
    ...
    @classmethod
    def from_config(cls, config: dict, char_vocab, phoneme_vocab) -> "G2PLSTMModel":
        """Constrói o modelo a partir de um dict de config (campo 'model' ou raiz).

        Usado por train.py e inference.py para evitar duplicação do bloco
        if/else panphon. O __init__ original não é alterado.
        """
