...
def build_panphon_features(phoneme_vocab) -> Dict[str, np.ndarray]:
    """Extrai feature matrix PanPhon a partir do vocab de fonemas.

    Encapsula a dependência de PanPhonPhonemeEmbedding dentro de losses.py,
    eliminando o import em train.py.

    Parameters
    ----------
    phoneme_vocab : PhonemeVocab
        Vocabulário de fonemas com atributos i2p (idx→str) e len().

    Returns
    -------
    Dict[str, np.ndarray]
        Mapeamento fonema → vetor de features (24D articulatório).
    """
    from phoneme_embeddings import PanPhonPhonemeEmbedding
    emb = PanPhonPhonemeEmbedding(
        vocab_size=len(phoneme_vocab),
        i2p_mapping=phoneme_vocab.i2p,
        padding_idx=0,
    )
    matrix = emb.feature_matrix.detach().cpu().numpy()
    return {phoneme_vocab.i2p[idx]: matrix[idx] for idx in range(len(phoneme_vocab))}
