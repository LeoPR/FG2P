...
**CrossEntropy é binária: certo (0) ou errado (1) — sem gradação**
...
Cenário B — Erro catastrófico (completamente errado):
  ...
  CrossEntropy: penalidade = 1.0  ← PROBLEMA: penalidade IDÊNTICA!
```

**O Problema**: Ambos recebem penalidade = 1.0, mesmo sendo muito diferentes!
→ Modelo não consegue aprender que ɛ→e (quase acerto) merecia penalidade menor
→ Trata erros de "quase acerto" igual a erros "catastróficos"
...
- **Cross Entropy (CE)**: Erro base — todos os erros têm mesma penalidade
