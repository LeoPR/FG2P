...
1. Benchmark multi-model executado em CPU+GPU com metricas ampliadas (w/s, tok/s, chars/s, CI95).
   - Comando: `python src/benchmark_inference.py --device both --warmup 8 --runs 40`
...
- A critica de "speedup possivelmente artificial por comparacao com experimento transitorio" foi valida para o estado textual inicial.
- O componente robusto de Exp106 segue sendo: hifen parece fonologicamente irrelevante (impacto pequeno em PER/WER).
- O componente de velocidade foi rebaixado para leitura prudente e condicionado a benchmark formal com IC95, sem claim universal de deployment speed-critical.

## Fechamento (2026-03-15)

1. Reclassificacao editorial aplicada: Exp106 permanece como ablation exploratoria de eficiencia, nao como perfil principal de latencia.
2. Fluxo de benchmark consolidado no script canônico: `src/benchmark_inference.py`.
3. Sweep formal CPU/GPU com IC95 documentado em `docs/benchmarks/BENCHMARK.md` e refletido no README.
4. Decisao de claim: nao sustentar "2,58x" como afirmacao geral; adotar narrativa por regime (hardware + batch + modelo).

## Decisao final da avaliacao

- Avaliacao encerrada como respondida.
- A narrativa correta e: ganhos de velocidade sao reais, mas condicionais ao regime de inferencia; nao ha um unico fator (Exp106) que sustente claim forte isolado de deployment.
